...
class ADS1256:
    def __init__(self):
        self.rst_pin = config_ADC.RST_PIN
        self.cs_pin = config_ADC.CS_PIN
        self.drdy_pin = config_ADC.DRDY_PIN

    # Hardware reset
    def ADS1256_reset(self):
        config_ADC.digital_write(self.rst_pin, GPIO.HIGH)
        config_ADC.delay_ms(200)
        config_ADC.digital_write(self.rst_pin, GPIO.LOW)
        config_ADC.delay_ms(200)
        config_ADC.digital_write(self.rst_pin, GPIO.HIGH)
    
    def ADS1256_WriteCmd(self, reg):
        config_ADC.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        config_ADC.spi_writebyte([reg])
        config_ADC.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
    
    def ADS1256_WriteReg(self, reg, data):
        config_ADC.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        config_ADC.spi_writebyte([CMD['CMD_WREG'] | reg, 0x00, data])
        config_ADC.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
        
    def ADS1256_Read_data(self, reg):
        config_ADC.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        config_ADC.spi_writebyte([CMD['CMD_RREG'] | reg, 0x00])
        data = config_ADC.spi_readbytes(1)
        config_ADC.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
    ...
    def ADS1256_WaitDRDY(self):
        #for i in range(0,400000,1):
            #if(config_ADC.digital_read(self.drdy_pin) == 0):
    ...
    #The config_ADCuration parameters of ADC, gain and data rate
    def ADS1256_config_ADCADC(self, gain, drate):
        self.ADS1256_WaitDRDY()
        buf = [0,0,0,0,0,0,0,0]
        buf[0] = (0<<3) | (1<<2) | (0<<1)
        buf[1] = 0x08
        ...
        config_ADC.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        config_ADC.spi_writebyte([CMD['CMD_WREG'] | 0, 0x03])
        config_ADC.spi_writebyte(buf)
        
        config_ADC.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
        config_ADC.delay_ms(1) 
    ...
    def ADS1256_init(self):
        ...
        if id == 3 :
            ...
            return -1
        self.ADS1256_config_ADCADC(ADS1256_GAIN_E['ADS1256_GAIN_1'], ADS1256_DRATE_E['ADS1256_7500SPS'])
        return 0
        
    def ADS1256_Read_ADC_Data(self):
        self.ADS1256_WaitDRDY()
        config_ADC.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        config_ADC.spi_writebyte([CMD['CMD_RDATA']])
        # config_ADC.delay_ms(10)

        buf = config_ADC.spi_readbytes(3)
        config_ADC.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
